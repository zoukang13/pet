				  ...
				  <div>
				  	<div style="margin-bottom: 15px;">
				  		...
				  		<el-upload
							  ...
							  action="https://www.ecnupet.cn:8080/pet/file"
...
export default{
  ...
  computed: {
  	imageUrl () {
  		if (this.image)
  			return  'https://www.ecnupet.cn:8080/pet/' + this.image
  		else
  			return ''
  	}
  },
  ...
  methods: {
      ...
      md5.update(vm.password)
  		this.$http.post(
	    	...
	    	utils.setCookie('username', res.data.user.name)
	    	utils.setCookie('picture_url',res.data.user.picture_url)
	    	utils.setCookie('userid',res.data.user.id)
	    	vm.loading1 = false
	    	vm.$router.push('/')
	    }).catch(function(err) {
	        console.log(err);
